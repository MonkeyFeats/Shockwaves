...
void Explode(CBlob@ this, f32 radius, f32 damage)
{
	...
	getRules().set_s32("timer", getGameTime()); //set the timer in kag.as
	Driver@ driver = getDriver();
	const f32 scalex = getDriver().getResolutionScaleFactor();
	const f32 zoom = getCamera().targetDistance * scalex;
	//driver.SetShaderFloat("shockwave", "radius", radius); //unused atm
	driver.SetShaderFloat("shockwave", "zoom", zoom);
	driver.SetShaderFloat("shockwave", "explosion_x", (Maths::Abs(-this.getScreenPos().x)/getScreenWidth()));
	driver.SetShaderFloat("shockwave", "explosion_y", 1.0 + (-this.getScreenPos().y/getScreenHeight()));
